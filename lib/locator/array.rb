class Array
  ...
  def to_region(options = {})
    if self[0].is_a?(String) and self[1].is_a?(String)
      if row = Locator.zip_codes.find {|row| row[3] == self[0].to_s and row[4] == self[1].to_s }
        if options[:city]
          return row[1]
        elsif options[:state]
          return row[2]
  ...
  def to_gmt_offset
    row = Locator.zip_codes.find {|row| row[3] == self[0].to_s and row[4] == self[1].to_s }
    row[5] if row
  end
